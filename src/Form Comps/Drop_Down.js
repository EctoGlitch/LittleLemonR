...
const Standalone_Drop_Down = ({ label, name, options, img_defualt, img_active, img_w, img_h, value: initialValue, onChange }) => {
    ...
    return (
        <>
            <div className='flex flex-col w-full' ref={dropdownRef}>
                {label && (
                    <label className='font-p text-white font-bold py-3'>
                        {label}
                    </label>
                )}
                <div className='w-full relative'>
                    <button
                        onClick={toggleDropdown}
                        className={`max-sm:w-full w-11/12 h-[80px] relative font-p py-6 rounded-4xl focus:outline-black appearance-none text-center flex items-center justify-between px-4 ${selectedOption === placeholderText ? 'text-black bg-[#fff]' : 'bg-gold text-black'}`}
                    >
                        <img
                            src={imageSrc}
                            alt="Dropdown Icon"
                    ...
                    {isOpen && (
                        <div className="absolute max-sm:w-full w-11/12 top-full left-0 right-0 bg-[#fff] rounded-4xl mt-2 z-20 shadow-lg drop-shadow-lg">
                            <div
                                onClick={handlePlaceholderClick}
                                className="px-4 py-3 font-p text-center cursor-pointer hover:bg-gray-100 text-black"
                            >
